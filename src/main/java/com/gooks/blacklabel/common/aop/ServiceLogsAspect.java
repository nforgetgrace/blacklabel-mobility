...
/**
* @package : com.gooks.blacklabel.common.aop
* @name : ServiceLogsAspect.java
* @date : 2022-12-14 오후 3:33
* @author : Jung Jae gook
* @version : 1.0.0
* @modifyed :
* @description : 서비스 로그처리
**/
...
public class ServiceLogsAspect {
    ...
    @Pointcut("within(com.gooks.blacklabel.api.*..*ServiceImpl)") // service 발생 오류 전체
    private void serviceLogsAspect() {
    }

    /* 어노테이션 정의 하지 않음 주석 풀면 Bean 에러남
    @Pointcut("@annotation(com.gooks.blacklabel.api.servlet.aop.custom.annotation.ServiceLogs)") // ServiceLogs 어노테이션 사용 클래스,메소드
    private void serviceLogs() {
    }
    */
    @Pointcut("within(com.gooks.blacklabel.api.*..*Controller)") // controller debug 로그
